...
const createStripePaymentSessionController = async (req, res) => {
  ...
  try {
    ...
    // Use dynamic success and cancel URLs if provided, otherwise use defaults
    const success_url = successUrl || "http://localhost:3000/payment-success"
    const cancel_url = cancelUrl || "http://localhost:3000/payment-cancelled"
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: Math.round(challan.Amount * 100), // Ensure it's an integer
          },
          quantity: 1,
        },
      ],
